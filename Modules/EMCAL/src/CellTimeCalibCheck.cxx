...
void CellTimeCalibCheck::configure()
{
  ...
  if (nThreshTSpectrum != mCustomParameters.end()) {
    try {
      ...
      ILOG(Error, Support) << fmt::format("Value {} not a double", nThreshTSpectrum->second.data()) << ENDM;
    }
  }
  ...
  if (nSigmaTSpectrum != mCustomParameters.end()) {
    try {
      ...
      ILOG(Error, Support) << fmt::format("Value {} not a double", nSigmaTSpectrum->second.data()) << ENDM;
    }
  }
}
...
void CellTimeCalibCheck::beautify(std::shared_ptr<MonitorObject> mo, Quality checkResult)
{
  ...
  //            //
  //            msg->Clear();
  //            msg->AddText("cellTimeCalib_PHYS increases with time: OK!!!");
